<template>
  ...
  <div id="chart_iden" style="height:600px;width:1400px;"></div>
</template>

<script setup name="Iden">
...
const init = () => {
  proxy.$http.get("/api/spectrum/getAllData/0").then(res => {
    data.value = res.data;
  })
}

const confirm = () => {
  proxy.$http.post("/api/spectrum/iden" ,
  ...
  .then(res => {
    let data = res.data;
    let legendData = ['混合物数据']
    let seriesData = [{name: '混合物数据', type: 'line', data: data.dbData}]
    data.idenData.forEach(iden => {
      legendData.push(iden.proportion + '-' + iden.name)
      seriesData.push({name: iden.proportion + '-' + iden.name, type: 'line', data: iden.curve})
    })

    echarts.init(document.getElementById('chart_iden')).dispose();
    var myChart = echarts.init(document.getElementById('chart_iden'));
    myChart.setOption({
      title: {
        text: '光谱数据'
      },
