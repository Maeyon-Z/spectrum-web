<template>
  ...
  <div id="chart_mixed" style="height:600px;width:1400px;"></div>
</template>

<script setup name="Mixed">
...
const confirm = () => {
  proxy.$http.post("/api/spectrum/test/0" ,
  ...
  .then(res => {
    let data = res.data;
    echarts.init(document.getElementById('chart_mixed')).dispose();
    var myChart = echarts.init(document.getElementById('chart_mixed'));
    myChart.setOption({
      title: {
        text: '光谱数据'
      },
